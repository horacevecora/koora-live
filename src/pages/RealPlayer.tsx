...
export default function RealPlayer() {
  ...
  const buildPlayer = useCallback(
    (server: Server, useProxy = false, shouldUnmute = hasInteracted) => {
      ...
      // التحويل التلقائي للبروكسي إذا كان مطلوباً (أو إذا فشلت المحاولة الأولى)
      ...
      if (isM3U8) {
        ...
        if (Hls.isSupported()) {
          ...
          hls.on(Hls.Events.ERROR, (event, data) => {
            if (data.fatal && !useProxy && originalUrl.startsWith('http')) {
              // فشل البث العادي -> جرب البروكسي تلقائياً وبصمت
              setTriedProxy(true);
              buildPlayer(server, true, shouldUnmute);
            } else if (data.fatal) {
              setError("البث غير متوفر حالياً، يرجى تجربة سيرفر آخر.");
              setLoading(false);
            }
          });
      ...
      // محاولة التشغيل المباشر (Raw Stream/TS)
      if (isRawStream && !finalUrl.includes("<iframe")) {
        ...
        video.onerror = () => {
          if (!useProxy && originalUrl.startsWith('http')) {
            ...
            setError("عذراً، هذا السيرفر لا يعمل حالياً.");
            setLoading(false);
          }
        };
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (!pageData) { setError("الصفحة غير موجودة"); return; }
        ...
        else setServers([{ name: "سيرفر 1", url: "https://8.wwwkora.com/albaplayer/bein-sports-hd-1/?serv=1", type: "iframe" }]);
  ...
  if (fetching) {
    return (
      <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center text-white">
        ...
        <p className="font-bold">جارٍ تحميل البث...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#020617] flex flex-col items-center pt-16 px-6 md:px-24 pb-6 font-sans relative overflow-hidden">
      ...
      <article id="main-player-wrapper" className="w-full max-w-[1200px] rounded-2xl bg-black flex flex-col relative border border-indigo-500/30 shadow-[0_0_25px_rgba(99,102,241,0.25)]">
        ...
        <div className="relative w-full bg-black aspect-video rounded-b-2xl overflow-hidden" onClick={handleUnmute}>
          ...
          {loading && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 z-10">
              ...
              <p className="mt-4 text-slate-300 text-sm font-bold">
                {triedProxy ? "جاري تشغيل البروكسي الآمن..." : "جارٍ استقرار البث..."}
              </p>
            </div>
          )}
      ...
      <div className="mt-12 w-full max-w-[1200px] overflow-x-auto no-scrollbar" dir="rtl">
        <div className="flex flex-nowrap justify-center gap-2 min-w-max px-4">
          {tags.map((tag, index) => (
            <span key={index} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] font-bold text-slate-500 hover:text-[#00e676] transition-colors cursor-default whitespace-nowrap">
              #{tag}
            </span>
          ))}
        </div>
      ...
      <footer className="w-full max-w-[1200px] mt-12 px-4 text-center" dir="rtl">
        <p className="text-[8px] text-slate-600 font-bold uppercase tracking-widest">
          Koora Live Official Player v4.0 - Automatic Secure Routing © 2026
        </p>
      </footer>
      ...
      <style>{`
        ...
        #main-player-wrapper:fullscreen { width: 100vw; height: 100vh; border-radius: 0; display: flex; align-items: center; justify-content: center; background: #000; border: none; }
